...
class MLPMixer(nn.Module):
    def __init__(self, seq_len, patch_size, acc_axis, gyr_axis, hidden_dim,
                 num_layers=4, expansion_factor=4, dropout=0.1, encoder_method="add"):
        ...
        if encoder_method == "cat":
            ...
            self.encoders = nn.Sequential(
                *[MLPMixerBlock(self.seq_len // self.patch_size, hidden_dim, expansion_factor, dropout)
                  for _ in range(num_layers)]
            )
...
def mlp_mixer(model_name: str, config: MLPMixerConfig):
    ...
    return MLPMixer(seq_len=config.seq_len, patch_size=config.patch_size,
                    ...
                    dropout=config.dropout, encoder_method=config.encoder_method)
