...
if __name__ == '__main__':
    ...
    train_mat = scio.loadmat(os.path.join(basic_config.dataset_path,
                                          ...
                                          'train.mat'))
    left_eval_mat = scio.loadmat(os.path.join(basic_config.dataset_path,
                                              '%s-%s-%d' %
                                              (basic_config.preprocess_method,
                                               basic_config.preprocess_strategy,
                                               basic_config.seq_len),
                                              'left_test.mat'))
    right_eval_mat = scio.loadmat(os.path.join(basic_config.dataset_path,
                                               '%s-%s-%d' %
                                               (basic_config.preprocess_method,
                                                basic_config.preprocess_strategy,
                                                basic_config.seq_len),
                                               'right_test.mat'))
    eval_mat = {
        'accData': np.vstack([left_eval_mat['accData'], right_eval_mat['accData']]),
        'gyrData': np.vstack([left_eval_mat['gyrData'], right_eval_mat['gyrData']]),
        'label': np.vstack([left_eval_mat['label'], right_eval_mat['label']]),
    }
